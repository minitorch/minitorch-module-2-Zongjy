...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t1,) = ctx.saved_values
        gs: Tensor = grad_output.f.sigmoid_map(t1)
        return gs.f.add_zip(gs, gs.f.mul_zip(gs, gs.f.neg_map(gs)))
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        reverse_od_list = [0 for _ in order._tensor._storage]
        for i, j in enumerate(order._tensor._storage):
            reverse_od_list[int(j)] = i
        new_tensordata = grad_output._tensor.permute(*reverse_od_list)
